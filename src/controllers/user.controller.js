...
console.log(req.body)
  const { firstName, lastName, email, password, confirmPassword, role,agreedToTerms } = req.body
    ...
    const newUser = new Users({ firstName, lastName, email, password, confirmPassword, role,agreedToTerms })
    const userCreated =  await newUser.save();
    if (!userCreated){
      return next(new ApiError(400, 'User is not Created try again', {error:message}))
    }
    res.status(201).json(new ApiResponse(200, Users, 'User created successfully'))
